...
1. To create a service namespace, visit the [Azure classic portal][Azure classic portal]. Click **Service Bus** on the left-hand side, then click **Create**. Type a name for your namespace, then click the check mark.
2. In the main window of the [Azure classic portal][Azure classic portal], click the name of the namespace you created in the previous step.
...
4. After the `<services>` element, create a `<behaviors>` element with the following content, replacing "SAS_KEY" with the *Shared Access Signature* (SAS) key you obtained from the [Azure classic portal][Azure classic portal] in Step 1.
